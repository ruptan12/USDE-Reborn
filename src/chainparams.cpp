...
{
    const char* pszTimestamp = "USDE Rebirth";
    const CScript genesisOutputScript = CScript() << ParseHex("REPLACE WITH NEW GENESIS BLOCK HASH USDE") << OP_CHECKSIG;
...
public:
    CMainParams() {
        ...
        vSeeds.emplace_back("node.whiterockserver.com", true);
...
public:
    CTestNetParams() {
        ...
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("INSERT GENESIS HASH"));
        assert(genesis.hashMerkleRoot == uint256S("INSERT MERKLE HASH"));
